...
describe('Test App Store', () => {
  ...
  describe('switchEnv', async () => {
    ...
    it('prodCheck: switches to production if AEM', async () => {
      ...
      const liveHost = `main--aem-boilerplate--adobe.${process.env.HLX_PROD_SERVER_HOST_LIVE}`;
      instance.siteStore.host = prodHost;
  ...
  describe('publish', () => {
    ...
    it('should handle successful publish', async () => {
      ...
      instance.status = { webPath: '/somepath' };
      instance.siteStore = {
        innerHost: `main--aem-boilerplate--adobe.${process.env.HLX_PROD_SERVER_HOST_PAGE}`,
        outerHost: `main--aem-boilerplate--adobe.${process.env.HLX_PROD_SERVER_HOST_LIVE}`,
        host: 'host',
      };
      instance.location = {
    ...
    it('should handle publish when outerHost is defined', async () => {
      sidekickTest.sandbox.stub(instance, 'isContent').returns(true);
      instance.status = { webPath: '/somepath' };
      instance.siteStore = { innerHost: `main--aem-boilerplate--adobe.${process.env.HLX_PROD_SERVER_HOST_PAGE}`, outerHost: `main--aem-boilerplate--adobe.${process.env.HLX_PROD_SERVER_HOST_LIVE}` };
      instance.location = { href: 'https://aem-boilerplate.com', host: 'aem-boilerplate.com' };
    ...
    it('should purge host', async () => {
      ...
      sidekickTest.sandbox.stub(instance, 'isEditor').returns(true);
      instance.status = { webPath: '/somepath' };
      instance.siteStore = { innerHost: `main--aem-boilerplate--adobe.${process.env.HLX_PROD_SERVER_HOST_PAGE}`, host: 'aem-boilerplate.com' };
      instance.location = { href: 'https://aem-boilerplate.com', host: 'aem-boilerplate.com' };
  ...
  describe('showView', () => {
    ...
    it('exits early if "path" search param is present', async () => {
      ...
      fetchMock.get(`https://admin.${process.env.HLX_PROD_SERVER_HOST_PAGE}/status/adobe/aem-boilerplate/main/path/placeholders.json?editUrl=auto`, {
        status: 200,
        body: {
          body: { },
        },
      ...
      findViewsSpy = sinon.spy(instance, 'findViews');
      instance.location = new URL(`https://main--aem-boilerplate--adobe.${process.env.HLX_PROD_SERVER_HOST_PAGE}/placeholders.json?path=/path/placeholders.json`);
    ...
    it('sets iframe src correctly if a DEFAULT view is found and no overlay exists', async () => {
      sidekickTest.mockFetchSidekickConfigSuccess(true, false);
      isProjectStub.returns(true);
      instance.location = new URL(`https://main--aem-boilerplate--adobe.${process.env.HLX_PROD_SERVER_HOST_PAGE}/placeholders.json`);
      ...
      expect(frame.src).to.equal(`http://viewer.com/?url=https%3A%2F%2Fmain--aem-boilerplate--adobe.${process.env.HLX_PROD_SERVER_HOST_PAGE}%2Fplaceholders.json&title=Test+Title`);
